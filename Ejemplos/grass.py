...
def install_deb(deb_path):
    ...
    try:
        # Intenta instalar el paquete .deb
        subprocess.run(f"sudo dpkg -i {deb_path}", shell=True, check=True)
        # Si hay dependencias faltantes, las resuelve
        subprocess.run("sudo apt -f install -y", shell=True, check=True)
        print("✅ Instalación exitosa.")
    except subprocess.CalledProcessError as e:
        print(f"❌ Error instalando el paquete .deb: {e}")
        print("❗ Intentando reparar dependencias...")
        try:
            subprocess.run("sudo apt --fix-broken install -y", shell=True, check=True)
        except subprocess.CalledProcessError as repair_error:
            print(f"❌ No se pudo reparar las dependencias: {repair_error}")

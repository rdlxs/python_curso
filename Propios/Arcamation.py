...
def extraer_datos_tipo3(doc: pymupdf.Document) -> FacturaResult:
    """
    Factura del jardín (tipo 3).

    Extrae los datos únicamente desde el texto embebido en el PDF.
    No utiliza OCR ni requiere Tesseract.
    """
    texto: str = ""

    # 1) Buscar la página que contiene la palabra "original".
    for p_any in doc:
        pagina = cast(pymupdf.Page, p_any)
        texto_pagina = cast(str, pagina.get_text("text"))

        if re.search(r"\boriginal\b", texto_pagina, flags=re.IGNORECASE):
            texto = texto_pagina
            break

    # 2) Si no apareció "original", buscar el marcador del jardín.
    if not texto:
        for p_any in doc:
            pagina = cast(pymupdf.Page, p_any)
            texto_pagina = cast(str, pagina.get_text("text"))

            if "RECREANDO INFANCIAS" in texto_pagina.upper():
                texto = texto_pagina
                break

    # 3) Como último recurso, procesar la primera página.
    if not texto:
        if len(doc) == 0:
            raise ValueError("PDF vacío: no hay páginas para procesar (tipo 3).")

        pagina = cast(pymupdf.Page, doc[0])
        texto = cast(str, pagina.get_text("text"))
    ...
    # Buscar el monto dentro del texto del PDF.
    monto_match = re.search(
        r"(?:importe\s+total|total\s+a\s+pagar|total)\s*:?"
        r"\s*\$?\s*([\d\.]+,\d{2}|[\d,]+\.\d{2})",
        texto,
        flags=re.IGNORECASE,
    )
...
def procesar_factura(path: Path) -> Optional[FacturaResult]:
    ...
    try:
        ...
        texto_completo = "\n".join(
            cast(str, cast(pymupdf.Page, p).get_text("text"))
            for p in doc
        )
